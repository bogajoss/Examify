...
export function cleanCsvHtml(text: string): string {
  ...
  text = text.replace(/(<font\b[^>]*?)\bcolor\s*=\s*["']?[^"'>]*?["']?([^>]*?>)/gi, function ($0, $1, $2) {
    return $1 + $2;
  });
 ...
 */
export function detectZeroIndexedAnswers(questions: any[]): boolean {
...
export async function parseCSV(file: File): Promise<ParsedQuestion[]> {
  ...
  const headers = headerLine.split(',').map(h => h.trim().toLowerCase().replace(/["\']|^\s+|\s+$/g, ''));

  // Find column indices
  const findColumnIndex = (names: string[], ...alternatives: string[][]): number => {
  ...
  const questionCol = findColumnIndex(headers, ['question', 'question_text', 'q']);
  const option1Col = findColumnIndex(headers, ['option1', 'option_1', 'opt1', 'a']);
  const option2Col = findColumnIndex(headers, ['option2', 'option_2', 'opt2', 'b']);
  const option3Col = findColumnIndex(headers, ['option3', 'option_3', 'opt3', 'c']);
  const option4Col = findColumnIndex(headers, ['option4', 'option_4', 'opt4', 'd']);
  const option5Col = findColumnIndex(headers, ['option5', 'option_5', 'opt5', 'e']);
  const answerCol = findColumnIndex(headers, ['answer', 'ans', 'correct_answer']);
  const explanationCol = findColumnIndex(headers, ['explanation', 'exp', 'explanation_text']);
  ...
  if (questionCol === -1 || option1Col === -1 || option2Col === -1 || option3Col === -1 || answerCol === -1) {
    throw new Error('CSV must have columns: question, option1, option2, option3, answer');
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    questions.push({
      ...
      option4: option4Col >= 0 ? cleanCsvHtml(values[option4Col] || '') : undefined,
      option5: option5Col >= 0 ? cleanCsvHtml(values[option5Col] || '') : undefined,
      answer: values[answerCol] || '',
      explanation: explanationCol >= 0 ? cleanCsvHtml(values[explanationCol] || '') : undefined,
      subject: subjectCol >= 0 ? values[subjectCol]?.trim() || undefined : undefined,
      paper: paperCol >= 0 ? values[paperCol]?.trim() || undefined : undefined,
      chapter: chapterCol >= 0 ? values[chapterCol]?.trim() || undefined : undefined,
      highlight: highlightCol >= 0 ? values[highlightCol]?.trim() || undefined : undefined,
